...
$(function() {
  $("[name=send]").click(function (e) {
   ...
   var name_tl = localStorage.name = form.find('input[name="name"]').val();
   ...
   if (!(error == 1)) {
    ...
    $.ajax({
      type: 'POST',
      url: 'https://api.telegram.org/bot363552321:AAE51TmXiKKOI08k69rTd76_c6Q07homRck/sendMessage?chat_id=-196934945&text=У вас новый лид c Yaburo LP: ' + name_tl + ' | ' + email_tl + ' | ' + phone_tl,
    });
    // Отправка на почту
    $.ajax({
      type: 'POST',
